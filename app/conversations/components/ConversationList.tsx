...
const ConversationList: React.FC<Props> = ({ initialConversations, users }) => {
    ...
    const asideStyles = clsx("fixed inset-y-0 pb-20 lg:pb-0 lg:left-20 lg:w-80 lg:block overflow-y-auto border-r border-gray-200", isOpen ? "hidden" : "block w-full left-0");
            ...
            <aside className={asideStyles}>
                <div className="px-5">
                    ...
                    {conversations.map((conversation) => (
                        <ConversationBox key={conversation.id} conversation={conversation} selected={conversationId === conversation.id} />
                    ))}
                </div>
            </aside>
        </>
